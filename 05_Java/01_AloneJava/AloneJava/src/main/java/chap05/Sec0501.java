 ...
 *      => 스택 방 : 주소(방번호) 값만 저장되고
 *      => 힙 방 : 값이 저장됨
 *      JVM(Java Virtual Machine) : 자바가상머신 : jdk (자바가상머신)
 *      역할 : linux/window(운영체제) 모든 곳에서 실행할 수 있게 만드는 프로그램
 *      비교연산자(==) : 기본자료형에서는 값을 비교하고
 *                     참고자료형에서는 주소(방번호)를 비교하게 됨
 *                     (스택방의 저장된 값을 비교함)
