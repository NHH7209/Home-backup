...
-- 직위      : JOB
-- 모든 컬럼 : *
SELECT * FROM EMPLOYEE WHERE JOB = 'MANAGER' ORDER BY ENAME ASC;
...
-- 사원명 : ENAME
-- 직위 : JOB
SELECT * FROM EMPLOYEE WHERE JOB = 'CLERK' ORDER BY ENAME DESC;
...
-- 사원테이블 : EMPLOYEE
SELECT ENAME, SALARY, SALARY * 12 + 100 AS "연간 총수입" FROM EMPLOYEE ORDER BY SALARY * 12 + 100 DESC;
...
BETWEEN 2000 AND 3000 AND DNO IN (20, 30) ORDER BY ENAME ASC;
...
-- 힌트) 조건 2개 AND 연결
SELECT * FROM EMPLOYEE WHERE DNO = 30 AND JOB = 'SALESMAN';
...
SELECT * FROM EMPLOYEE WHERE DNO IN (20, 30) AND SALARY > 2000;
...
SELECT * FROM EMPLOYEE WHERE ENAME LIKE '%E%' AND DNO = 30 AND SALARY NOT BETWEEN 1000 AND 2000;
...
SELECT * FROM EMPLOYEE WHERE COMMISSION IS NULL AND 
